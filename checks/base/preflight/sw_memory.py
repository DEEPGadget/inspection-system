...
def check_memory_errors():
    ...
    try:
        ...
        try:
            result = subprocess.run(
                ["edac-util", "-s", "0"], capture_output=True, text=True, timeout=5
            )
            if result.returncode == 0:
                error_count = 0
                for line in result.stdout.split("\n"):
...
def main():
    try:
        ...
        result = {"check": "sw_memory", "status": status, "detail": detail}
        ...
        result = {"check": "sw_memory", "status": "fail", "detail": "error=exception"}
        print(json.dumps(result))
