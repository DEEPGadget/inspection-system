...
def get_dmesg_errors():
    ...
    try:
        ...
        result_err = subprocess.run(
            ["dmesg", "--level=err,crit,alert,emerg"], capture_output=True, text=True, timeout=10
        )
        error_lines = 0
        ...
        result_warn = subprocess.run(
            ["dmesg", "--level=warn"], capture_output=True, text=True, timeout=10
        )
        warn_lines = 0
...
def main():
    try:
        ...
        result = {"check": "collect_all_logs", "status": status, "detail": detail}
        ...
        result = {"check": "collect_all_logs", "status": "fail", "detail": "error=exception"}
        print(json.dumps(result))
