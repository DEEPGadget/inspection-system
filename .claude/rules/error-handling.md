...
- **Preflight baseline 설치 실패는 즉시 `failed`** (silent 진행 금지). `_install_and_verify_baseline()`이 `dpkg -s`로 패키지별 검증 → 1개라도 누락 시 `check_results`에 `baseline_install` 항목으로 `installed/missing/apt_tail` 기록 후 `_mark_failed` + `_dispatch_cleanup`. `sudo_password` 미제공 케이스도 동일.
